...
class SmsFrameworkGatewayAdminTest extends SmsFrameworkBrowserTestBase {
  ...
  public function testGatewayEdit(): void {
    ...
    $this->assertSession()->fieldExists('retention_duration_incoming', '0');
    $this->assertSession()->fieldExists('retention_duration_outgoing', '0');

    // Memory gateway supports pushed reports, so the URL should display.
    $this->assertSession()->fieldExists('delivery_reports[push_path]', $test_gateway->getPushReportPath());
  ...
  public function testGatewayDelete(): void {
    ...
    $this->assertSession()->addressEquals($delete_url->toString());
    ...
    $this->assertSession()->addressEquals(Url::fromRoute('sms.gateway.list')->toString());
    $this->assertSession()->statusCodeEquals(200);
